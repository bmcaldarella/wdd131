...
const sweaterProducts = products.filter(product => product.category.toUpperCase() === "SWEATER");

const productContainer = document.getElementById('sweter-container');
sweaterProducts.forEach(product => {
    const container = document.createElement('div');
    container.classList.add('product');

    const productName = document.createElement('h2');
    productName.innerHTML = `<p class="productName">${product.productName}</p>`;

    const price = document.createElement('p');
    price.innerHTML = `<p class="price">Price: $${product.price}</p>`;
    ...
    firstImage.alt = product.productName;
